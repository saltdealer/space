...
_TAR_FILE = 'dist-myfun.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/myfun'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-myfun-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick myfun> %s' % (db_user, db_password, f))
...
def deploy_flush():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        sudo('mysql -uroot -p%s myfun < www/schema.sql' % db_password)
    with settings(warn_only=True):
        sudo('supervisorctl stop myfun')
        sudo('supervisorctl start myfun')
