...
public class CardContainer extends AdapterView<ListAdapter> {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		Log.i("the rect before evnet is ",childRect.toShortString());
	//	Log.d("Touch Event", MotionEvent.actionToString(event.getActionMasked()) + " ");
		...
		switch (event.getActionMasked()) {
			case MotionEvent.ACTION_DOWN:
				mTopCard.getHitRect(childRect);
				Log.i("the rect before click is ",childRect.toShortString());
			...
			//	mTopCard.setPivotY(points[1]);
				mTopCard.getHitRect(childRect);
				Log.i("the rect after click is ",childRect.toShortString());
			...
			case MotionEvent.ACTION_MOVE:
				...
				mTopCard.getHitRect(childRect);
				Log.i("the rect is before rotation ",childRect.toShortString()+"  "+mTopCard.getTranslationX() +"  "+dx);
				mTopCard.setTranslationX(mTopCard.getTranslationX() + dx);
				mTopCard.setTranslationY(mTopCard.getTranslationY() + dy);

				mTopCard.getHitRect(childRect);
				Log.i("the rect is after rotation 1",childRect.toShortString());
				//mTopCard.setRotation(40 * mTopCard.getTranslationX() / (getWidth() / 2.f));
				mTopCard.getHitRect(childRect);
				Log.i("the rect is after rotation ",childRect.toShortString());
		...
		mTopCard.getHitRect(childRect);
		Log.i("the rect after evnet is ",childRect.toShortString());
		return true;
	}
		...
		switch (event.getActionMasked()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (Math.abs(x - mLastTouchX) > mTouchSlop || Math.abs(y - mLastTouchY) > mTouchSlop) {
					...
					mTopCard.getHitRect(childRect);
					Log.i("the rect is befor move ",childRect.toShortString());
					mTopCard.getMatrix().invert(mMatrix);
					mMatrix.mapPoints(points);
					mTopCard.setPivotX(points[0]);
					mTopCard.setPivotY(points[1]);
					mTopCard.getHitRect(childRect);
					Log.i("the rect is after move ",childRect.toShortString());
					return true;
				}
			
		}
		mTopCard.getHitRect(childRect);
		Log.i("the rect after onInterceptTouchEvent is ",childRect.toShortString());
